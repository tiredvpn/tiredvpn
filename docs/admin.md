...
| `-tun-ip` | server-assigned | Fixed TUN IP for this client |
| `-max-conns` | server default | Max concurrent connections (0 = server default) |
| `-expires-in` | never | Expiry as a Go duration, e.g. `720h` |
| `-quic` | `true` | Include QUIC in the generated connection string |
| `-quic-port` | `8443` | QUIC port in the connection string |
| `-strategy` | `auto` | Strategy in the connection string |
| `-cover` | `api.googleapis.com` | Cover host in the connection string |

The client ID and secret are generated by the server and returned in the API
response. The command prints them along with a ready-to-use connection string.
...
The list does not include the client `secret`.
...
Only `name` is required; `tun_ip`, `max_conns`, and `expires_in` are optional.
The server generates the `id` and `secret` and returns them in the response,
which includes the new client's full config.
