...
Chart version 0.3.8, `appVersion` 1.9.0.

## Install

```bash
helm install my-tiredvpn oci://ghcr.io/tiredvpn/charts/tiredvpn \
  --version 0.3.8 -f my-values.yaml
```
...
Verified against the templates and `values.yaml` at chart 0.3.8. Each of these
...
- tiredvpn: `appVersion` in Chart.yaml, currently `1.9.0`. CI keeps it in step with the application release — a stable `v*` tag bumps `appVersion` to that version and patch-bumps the chart `version`.
