...
Chart version 0.3.14, `appVersion` 1.11.2.
...
Verified against the templates and `values.yaml` at chart 0.3.9. Each of these
...
- tiredvpn: `appVersion` in Chart.yaml, currently `1.11.2`. CI keeps it in step with the application release — a stable `v*` tag bumps `appVersion` to that version and patch-bumps the chart `version`.
