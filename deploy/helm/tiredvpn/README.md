...
Chart version 0.3.1, `appVersion` 1.5.1.
...
- tiredvpn: `appVersion` in Chart.yaml, currently `1.5.1`. CI keeps it in step with the application release — a stable `v*` tag bumps `appVersion` to that version and patch-bumps the chart `version`.
