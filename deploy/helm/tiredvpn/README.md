...
Chart version 0.3.4, `appVersion` 1.7.1.

## Install

```bash
helm install my-tiredvpn oci://ghcr.io/tiredvpn/charts/tiredvpn \
  --version 0.3.4 -f my-values.yaml
```
...
Verified against the templates and `values.yaml` at chart 0.3.4. Each of these
...
- tiredvpn: `appVersion` in Chart.yaml, currently `1.7.1`. CI keeps it in step with the application release — a stable `v*` tag bumps `appVersion` to that version and patch-bumps the chart `version`.
